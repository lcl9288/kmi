...
import { toWebpackLikeStats } from './util'
...
export async function build(opts: IOpts): Promise<void> {
  ...
  try {
    const viteRes = await viteBuilder(viteBuildConfig)
    result.stats = toWebpackLikeStats(viteRes, +new Date() - startTms, opts.cwd)
